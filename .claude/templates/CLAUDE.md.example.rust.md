...
> **本 file は `install.sh` が manifest 検出 (lang=rust) 後に auto-fill 生成した starter です。**
...
- **Language / Framework**: Rust{{FRAMEWORK_LINE}}
- **Runtime / Edition**: {{RUNTIME_LINE}}
- **Build Tool**: cargo (workspace 構成の場合は `[workspace]` を Cargo.toml で管理)
- **DB**: <!-- TODO(auto-fill): 例 PostgreSQL (sqlx) / SQLite / RocksDB -->
- **Async Runtime**: <!-- TODO(auto-fill): 例 tokio / async-std -->
- **Hosting**: <!-- TODO(auto-fill): 例 AWS / Fly.io / bare metal -->
- **AI / 外部 API**: <!-- TODO(auto-fill): 例 OpenAI, WASM host -->

## Architecture / Data

<!-- TODO(auto-fill): crate 構成 / 主要 module / docs リンク (例: docs/architecture.md) -->
