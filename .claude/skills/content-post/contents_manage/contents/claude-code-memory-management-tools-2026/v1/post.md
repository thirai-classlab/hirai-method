...
> Claude Code（および MCP 互換 AI コーディングエージェント）の「文脈喪失」を解決するための 8 ツール横断比較。ジュニアエンジニア／テックリード／非エンジニアそれぞれが「どのツールを、いつ、なぜ採用するか」を判断できるレファレンス。
...
<img src="https://d2f75plg0t6qwk.cloudfront.net/knowledge/claude-code-memory-management-tools-2026/970e9d0d-scene-01-context-loss.png" alt="文脈喪失を抱える素の LLM と、メモリレイヤを介して継続できる Claude の対比" width="1536" height="864">
...
```mermaid
flowchart LR
    subgraph 従来["素の Claude / LLM"]
      A1[会話開始] --> A2[context window]
      A2 --> A3[セッション終了]
      A3 --> A4[全て忘却]
    end
    subgraph 解決["メモリ管理ツールあり"]
      B1[会話開始] --> B2[context window]
      B2 --> B3[永続層へ書き出し]
      B3 --> B4[セッション終了]
      B4 --> B5[次回起動]
      B5 --> B6[関連だけ再ロード]
...
| **PM・非エンジニア** | Claude Code で議事録・FAQ・顧客対応スクリプトを継続的に育てる。"前回の私の好み" を覚えてもらう |
...
```mermaid
flowchart TB
    User([Claude Code / Cursor / Windsurf etc.])

    subgraph L1["ファイル層（人間が直接読む）"]
      C1[CLAUDE.md 純正階層]
      S1[Serena project memories]
    end

    subgraph L2["MCP サーバ層（ツール経由で読み書き）"]
      M1[MCP Memory Server<br/>知識グラフ JSONL]
      MP1[MemPalace<br/>verbatim + 索引]
      OM1[OpenMemory MCP<br/>ローカル横断記憶]
      SE1[Serena MCP<br/>LSP シンボル]
    end

    subgraph L3["タスク／DAG 層"]
      BD1[Beads bd<br/>git-backed issue DAG]
    end

    subgraph L4["エージェント自身が編集する層"]
      LE1[Letta<br/>self-editing memory]
      ME1[Mem0<br/>vector + KG dual store]
    end
...
<img src="https://d2f75plg0t6qwk.cloudfront.net/knowledge/claude-code-memory-management-tools-2026/251c184f-scene-02-taxonomy.png" alt="ファイル層 / MCP サーバ層 / タスク DAG 層 / エージェント基盤層の 4 タイプ分類" width="1536" height="864">
...
```mermaid
gantt
    title 各ツールの「無料で使える範囲」
    dateFormat X
    axisFormat %s
    section CLAUDE.md
    完全無料 :0, 100
    section MCP Memory Server
    完全無料 OSS :0, 100
    section Beads
    完全無料 OSS :0, 100
    section MemPalace
...
```mermaid
flowchart TD
    E[Enterprise Policy] --> P[Project CLAUDE.md]
    P --> R[Project Rules .claude/rules]
    R --> U[~/.claude/CLAUDE.md User]
    U --> A[Auto memory<br/>Claude が自己追記]
    style E fill:#ffebee
    style A fill:#e8f5e9
```
...
- Auto memory 機能で Claude 自身が `MEMORY.md` インデックスに追記する
...
```mermaid
flowchart LR
    User[ユーザー発言] --> Ent[Entity 作成]
    Ent --> Rel[Relation 作成]
    Rel --> Obs[Observation 蓄積]
    Obs --> JSONL[(memory.json<br/>JSONL ローカル)]
    JSONL --> Query[次セッションで検索]
...
- **非エンジニア**: パートナー会社・契約条件・連絡窓口を entity 化して Claude に問い合わせ
...
```mermaid
flowchart TD
    bd1[bd-a1b2 設計] --> bd2[bd-c3d4 実装]
    bd2 --> bd3[bd-e5f6 テスト]
    bd1 --> bd4[bd-g7h8 ドキュメント]
    bd3 --> bd5[bd-i9j0 デプロイ]
    bd4 --> bd5
    style bd1 fill:#fff9c4
    style bd5 fill:#c8e6c9
```
...
- **非エンジニア**: 「申込フォーム改修」のような長期 PJ で Claude に進捗管理を任せる
...
<img src="https://d2f75plg0t6qwk.cloudfront.net/knowledge/claude-code-memory-management-tools-2026/edc0150c-scene-03-axes.png" alt="コードの記憶 / 会話の記憶 / タスクの記憶 — 3 軸でツールを位置づける" width="1536" height="864">
...
```mermaid
flowchart LR
    Conv[会話原文] --> Verbatim[verbatim 保存<br/>要約しない]
    Verbatim --> Wing[Wing: 人/プロジェクト]
    Wing --> Room[Room: トピック]
    Room --> Drawer[Drawer: 原文格納]
    Drawer --> Search[semantic search で取り出し]
    style Verbatim fill:#fff3e0
...
- **非エンジニア**: 顧客対応スクリプトを逐語保存。トーン・敬語表現を学習素材化
...
```mermaid
flowchart TD
    Code[コードベース] --> LSP[Language Server]
    LSP --> Sym[シンボル抽出<br/>function/class/method]
    Sym --> Index[シンボル索引]
    Index --> Mem[Project memories MD]
    Mem --> Agent[Claude / Codex / Cursor]
    Agent -->|find_symbol<br/>rename_symbol| LSP
...
stateDiagram-v2
    [*] --> Core: 起動
    Core --> Working: ツール呼び出し
    Working --> Archival: 古い情報退避
    Archival --> Core: recall 検索
    Core --> SelfEdit: メモリ更新ツール
    SelfEdit --> Core: 自己改変
    Core --> [*]: REST API レスポンス
    note right of SelfEdit
      エージェントが
      自分でメモリを書き換える
    end note
```
...
- **非エンジニア**: 業務の自動応答エージェント（FAQ 応答→エスカレ判断）
...
```mermaid
flowchart LR
    Msg[会話メッセージ] --> Extract[fact 抽出 pipeline]
    Extract --> Vec[(ベクトル DB)]
    Extract --> KG[(知識グラフ)]
    Vec --> Search1[類似検索]
    KG --> Search2[関係検索]
    Search1 --> Ret[hybrid retrieval]
    Search2 --> Ret
...
- **非エンジニア**: 顧客プロフィール（過去契約・好み・連絡時間帯）を覚える接客 AI
...
flowchart TB
    subgraph Local["ローカルマシン"]
      OM[(OpenMemory<br/>ローカル DB)]
      Dash[Dashboard GUI]
    end
    OM --- Dash
    CC[Claude Code] -.MCP.-> OM
    CD[Claude Desktop] -.MCP.-> OM
    CR[Cursor] -.MCP.-> OM
    WS[Windsurf] -.MCP.-> OM
...
```mermaid
quadrantChart
    title 機能リッチさ × ベンダーロックイン
    x-axis シンプル --> 高機能
    y-axis ロックインなし --> ベンダーロックインあり
    quadrant-1 高機能 + ロックイン強
    quadrant-2 高機能 + 自由
    quadrant-3 シンプル + 自由
    quadrant-4 シンプル + ロックイン強
    CLAUDE.md: [0.15, 0.15]
    MCP Memory: [0.35, 0.1]
    Beads: [0.5, 0.15]
    OpenMemory: [0.55, 0.2]
...
```mermaid
flowchart TD
    Start[まず何が要る?] --> Q1{文脈喪失の頻度は?}
    Q1 -->|たまに| Cheap[CLAUDE.md だけで十分]
    Q1 -->|頻繁| Q2{何を覚えたい?}
    Q2 -->|規約・嗜好| CC[CLAUDE.md + MCP Memory]
    Q2 -->|コード構造| Ser[Serena]
    Q2 -->|長期 TODO| Bd[Beads]
    Q2 -->|会話原文| MP[MemPalace]
    Q2 -->|ユーザー好み| Q3{ツール跨ぎ?}
    Q3 -->|単一ツール| Mem[Mem0 OSS]
    Q3 -->|複数ツール| OM[OpenMemory MCP]
    Q2 -->|自律エージェント| Le[Letta self-host]
...
<img src="https://d2f75plg0t6qwk.cloudfront.net/knowledge/claude-code-memory-management-tools-2026/7fa742ad-scene-04-personas.png" alt="ジュニアエンジニア / テックリード / 非エンジニア — 3 つの立場とそれぞれに合うツール" width="1536" height="864">
...
| **ジュニアエンジニア** | 単一リポジトリで Claude Code を使い倒す | CLAUDE.md + Serena |
...
| **非エンジニア** | 顧客プロフィール記憶 AI を試作（個人情報取扱に注意） | Mem0 OSS（self-host） |
...
| 既に稼働中 | 内容 | 拡張余地 |
|---|---|---|
| 雑務ワークスペースの `CLAUDE.md` + `.claude/memory/` | 階層メモリ + Auto memory（feedback / project / reference 等） | Enterprise 層追加（会社ポリシー）、`.claude/rules/` 共通化 |
| Serena MCP | シンボル検索・編集の MCP として稼働中 | 各プロジェクトの memory MD を積極利用 |

---

<img src="https://d2f75plg0t6qwk.cloudfront.net/knowledge/claude-code-memory-management-tools-2026/47f9ab42-scene-05-decision.png" alt="採用判断の分岐 — 案件特性に応じて最適なツールを選ぶ" width="1536" height="864">
...
```mermaid
flowchart TD
    A[新規 PJ で Claude Code を使う] --> B{1ヶ月以内の小規模?}
    B -->|Yes| C[CLAUDE.md のみで開始]
    B -->|No| D{コード理解が主目的?}
    D -->|Yes| E[CLAUDE.md + Serena]
    D -->|No| F{長期タスク計画?}
    F -->|Yes| G[+ Beads]
    F -->|No| H{ツール横断 必要?}
    H -->|Yes| I[+ OpenMemory MCP]
    H -->|No| J{構造化された知識が要る?}
    J -->|Yes| K[+ MCP Memory Server]
    J -->|No| L{エージェント自律稼働?}
    L -->|Yes| M[+ Letta]
    L -->|No| N{原文ニュアンス保持?}
    N -->|Yes| O[+ MemPalace]
...
https://bruton.ai/blog/ai-trends/beads-bd-missing-upgrade-your-ai-coding-agent-needs-2026
...
https://www.pulsemcp.com/servers/modelcontextprotocol-memory
