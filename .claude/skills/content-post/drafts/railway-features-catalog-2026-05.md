---
title: "Railway 全機能カタログ 2026 — Push → URL を 1 画面で完結する agent-native cloud の全体像"
type: knowledge
category: reference
slug: railway-features-catalog-2026-05
thumbnail: ./images/railway-features-catalog-2026-05/thumbnail.png
author: "平井拓真"
difficulty: intermediate
summary: "Railway は『ソースコードを push したら数秒で URL が返ってくる』体験を、フロントエンドから DB / Cron / Object Storage / 自社ハードウェアまで含めて 1 つの管理画面に集約した agent-native cloud。Vercel が『フロントエンドの Vercel』なら、Railway は『バックエンドの Vercel』のポジションを取りに来ている。2026-01 に Series B $100M を調達し、Railway Metal（自社インフラ beta）/ Functions（canvas TS デプロイ）/ Buckets GA（S3 互換）/ Magic Config（AI 設定生成）/ Restricted Environments / Amsterdam EU リージョン追加など、訓練データが追いついていない新機能を連続投入中。本カタログは Trial / Hobby / Pro / Enterprise の 4 プラン体系から、デプロイ・ビルド / ランタイム / DB / ストレージ / 環境・コラボ / 観測性 / セキュリティ / API・拡張 の 8 カテゴリ 38 機能、料金構造、Render / Fly.io / Vercel / Heroku / AWS App Runner / Cloudflare Workers / Netlify / DO App Platform / Northflank / Coolify の 10 社競合比較、採用判断フローまで一気に整理する。"
---
...
![なぜ存在するか](./images/railway-features-catalog-2026-05/inline/r01.png)
...
![サービス全体俯瞰](./images/railway-features-catalog-2026-05/inline/r02.png)

### 2.2 製品カテゴリ mindmap

![製品カテゴリ mindmap](./images/railway-features-catalog-2026-05/inline/r03.png)

---

## 3. プラン体系の前提知識

### 3.0 課金単位の前提 — workspace / project / seat の階層（最重要）

**Railway の課金は project 単位ではない。`workspace` 単位 + Pro のみ seat 加算が乗る、というのが基本式**。Heroku / Vercel から来ると勘違いしやすいポイントなので最初に押さえる。

#### 階層構造

```text
Account（ログインユーザー）
└── Workspace（請求単位） ← サブスク料金 + リソース credit はここに紐付く
    ├── Project A（複数 Service / DB / Volume）
    ├── Project B
    └── Project C   ← 同 workspace 内なら何個作っても 月額固定費は増えない
```

- 1 アカウントは **複数 workspace に所属可能**（個人 workspace + 会社 workspace など）
- 1 workspace は **任意個数の project を持てる**
- 課金請求書は **workspace ごとに 1 通発行**

#### 課金式（プラン別）

| プラン | 月額（固定） | リソース credit | 課金単位 |
|---|---|---|---|
| **Trial** | $0 | $5 / 一回（30 日有効） | workspace（1 seat 固定） |
| **Hobby** | **$5 / workspace** | $5 / workspace | workspace（1 seat 固定、メンバー追加不可） |
| **Pro** | **$20 / seat / workspace** | $20 / seat / workspace（合算） | **workspace の seat 数 × $20**（メンバー追加で月額増加） |
| **Enterprise** | 契約額 / workspace | 契約 credit | workspace 単位（契約条件次第） |

**ポイント**:

- Project を増やしても **月額固定費は変わらない**（リソース使用量が増えるだけ）
- Pro で 3 人チームを workspace に追加 → 月額 $60、credit 合算で $60 分（= $20 × 3）の resource credit が workspace に入る
- workspace 内の **全 project の resource 使用量が合算され**、credit を超えた分が従量課金
- 個人で複数 project を試したいだけなら **Hobby 1 つで全部入れられる**（追加 workspace 不要）

#### よくある誤解

- ❌「project ごとに月額が発生する」 → **正**: workspace 単位の課金。project は中身の論理グルーピング
- ❌「Pro なら seat 課金は固定」 → **正**: seat 数 × $20。10 人なら $200/月
- ❌「個人で複数 project = 複数 Hobby 契約が必要」 → **正**: 1 Hobby workspace に複数 project を相乗りで OK
- ❌「Hobby にメンバー招待できる」 → **正**: Hobby は 1 seat 固定。チームを入れたいなら Pro に上げる

#### 実例で見るコスト感

| 利用パターン | 必要な構成 | 月額固定 | リソース credit |
|---|---|---|---|
| 個人で 5 つの趣味 project をまとめてホスト | Hobby × 1 workspace（project 5 個相乗り）| $5 | $5 / workspace |
| 3 人スタートアップで本番 + staging | Pro × 1 workspace × 3 seat（project 2 個）| $60 | $60 / workspace |
| 10 人受託会社で案件ごとに project | Pro × 1 workspace × 10 seat（project 多数）| $200 | $200 / workspace |
| 別事業を完全分離したい | Pro × 2 workspace（請求も別）| $20 ×（seat × workspace 数）| 各 workspace ごと |
...
![課金モデルの考え方](./images/railway-features-catalog-2026-05/inline/r04.png)
...
![Git Push Deploy](./images/railway-features-catalog-2026-05/inline/r05.png)
...
![Railpack](./images/railway-features-catalog-2026-05/inline/r06.png)
...
![Nixpacks](./images/railway-features-catalog-2026-05/inline/r07.png)
...
![Dockerfile ビルド](./images/railway-features-catalog-2026-05/inline/r08.png)
...
![Functions](./images/railway-features-catalog-2026-05/inline/r09.png)
...
![Service](./images/railway-features-catalog-2026-05/inline/r10.png)
...
![Auto Scale](./images/railway-features-catalog-2026-05/inline/r11.png)
...
![Replicas](./images/railway-features-catalog-2026-05/inline/r12.png)
...
![Cron Jobs](./images/railway-features-catalog-2026-05/inline/r13.png)
...
![Railway Metal](./images/railway-features-catalog-2026-05/inline/r14.png)
...
![PostgreSQL](./images/railway-features-catalog-2026-05/inline/r15.png)
...
![MySQL](./images/railway-features-catalog-2026-05/inline/r16.png)
...
![Redis](./images/railway-features-catalog-2026-05/inline/r17.png)
...
![MongoDB](./images/railway-features-catalog-2026-05/inline/r18.png)
...
![Custom DB](./images/railway-features-catalog-2026-05/inline/r19.png)
...
![Volumes](./images/railway-features-catalog-2026-05/inline/r20.png)
...
![Buckets](./images/railway-features-catalog-2026-05/inline/r21.png)
...
![Private Network](./images/railway-features-catalog-2026-05/inline/r22.png)
...
![Custom Domain + TLS 自動](./images/railway-features-catalog-2026-05/inline/r23.png)
...
![Edge / CDN](./images/railway-features-catalog-2026-05/inline/r24.png)
...
![Environments](./images/railway-features-catalog-2026-05/inline/r25.png)
...
![Preview Environments](./images/railway-features-catalog-2026-05/inline/r26.png)
...
![Restricted Environments](./images/railway-features-catalog-2026-05/inline/r27.png)
...
![Variables / Secrets 管理](./images/railway-features-catalog-2026-05/inline/r28.png)
...
![Magic Config](./images/railway-features-catalog-2026-05/inline/r29.png)
...
![Logs](./images/railway-features-catalog-2026-05/inline/r30.png)
...
![Metrics](./images/railway-features-catalog-2026-05/inline/r31.png)
...
![Webhooks](./images/railway-features-catalog-2026-05/inline/r32.png)
...
![Healthchecks](./images/railway-features-catalog-2026-05/inline/r33.png)
...
![SSO / SAML](./images/railway-features-catalog-2026-05/inline/r34.png)
...
![Audit Log](./images/railway-features-catalog-2026-05/inline/r35.png)
...
![SOC 2 / HIPAA](./images/railway-features-catalog-2026-05/inline/r36.png)
...
![IP Allowlist](./images/railway-features-catalog-2026-05/inline/r37.png)
...
![Railway CLI v4](./images/railway-features-catalog-2026-05/inline/r38.png)
...
![Public GraphQL API](./images/railway-features-catalog-2026-05/inline/r39.png)
...
![Template Marketplace](./images/railway-features-catalog-2026-05/inline/r40.png)
...
![GitHub App](./images/railway-features-catalog-2026-05/inline/r41.png)
...
![Terraform Provider](./images/railway-features-catalog-2026-05/inline/r42.png)
...
![Terraform Provider](./images/railway-features-catalog-2026-05/inline/r43.png)
...
![Terraform Provider](./images/railway-features-catalog-2026-05/inline/r44.png)

### 8.2 採用適性 Quadrant

![Terraform Provider](./images/railway-features-catalog-2026-05/inline/r45.png)
