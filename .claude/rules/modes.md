...
2. **中間確認の停止**: 「進めてもよいですか?」「どちらにしますか?」等の質問は禁止
   - **例外 (task-21 W2.1)**: 以下は禁止対象外、引き続き user 確認必須:
     ...
     - **規範変更** (`.claude/rules/*.md` / `.claude/commands/*.md` / `.claude/templates/docs/**/*.md` への Edit/Write) — user 確認必須 / 設計→承認フロー推奨は維持 (**honor system**)。**2026-05-28 緩和で `draft-flow-guard.sh` の機械強制 BLOCK は撤廃**され、これらの規範文書 path は新規 Write / 既存 Edit とも hook で PASS する。機械強制ではなく規律として残す方針
   ...
   - 起源 (規範変更): 2026-05-26 task-40 Step 5、本 session 規範違反 (規範変更時 draft skip) の再発防止 → 2026-05-28 緩和で機械強制 BLOCK は撤廃 (規律方針は honor system として維持)
