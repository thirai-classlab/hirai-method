...
feature_ui_contract_enabled	feature_toggle	ui-contract.sh (PostToolUse Edit|Write、UI 拡張子 .tsx/.jsx/.vue/.svelte/.html/.css/.scss 検出時 cross-file-contract-check.sh を呼出し id/symbol/API 名 drift を advisory WARN、task-98)	false にすると ui-contract.sh を OFF にし UI 拡張子変更後の cross-file 契約検証 hook が止まる (BLOCK しない advisory 方針)	UI契約hook有効化
feature_observability_enabled	feature_toggle	lib/observability.sh 5 API (log_event/log_block/log_bypass/log_fire/log_silent_failure) 経由の全 hook fire/block/bypass/silent_failure log append + observability-gc.sh 30 日 GC + hook-fire-audit.sh fire 0 回検出 (task-99)	false にすると全 log_* を no-op 化し observations.jsonl への observability event append が停止する (fire 0 回 hook 棚卸し unavailable)	observability有効化
feature_yml_triplet_check_enabled	feature_toggle	.claude/templates/githooks/pre-commit §4.5 yml triplet policy (harness-config.yml に新規 top-level key を追加した commit で consumer + smoke の三点揃を強制、BLOCK、task-100)	false にすると pre-commit §4.5 の yml triplet BLOCK が停止し新規 key 追加時の consumer / smoke 不在 commit が通る (規範値の「飾り化」再発リスク上昇)	yml三点揃検証有効化
feature_init_tasks_enabled	feature_toggle	init-tasks-on-start.sh の SessionStart bootstrap dispatcher-manifest 用 alias (feature_init_tasks_on_start_enabled と連動、task-104 W1-8)	false にすると dispatcher が init-tasks-on-start を spawn しなくなり task 台帳自動生成が止まる	タスク台帳init有効化
feature_why_x5_reminder_enabled	feature_toggle	why-x5-reminder.sh の SessionStart bootstrap dispatcher-manifest 用個別 toggle (feature_why_x5_enforcement_enabled parent の child、task-104 W1-8)	false にすると SessionStart の Why x5 reminder pointer 注入のみ止まる (violation-detect 側は parent toggle で継続)	Whyx5reminder有効化
feature_next_actions_surface_enabled	feature_toggle	next-actions-surface.sh の SessionStart bootstrap dispatcher-manifest 用個別 toggle (feature_byproduct_discharge_enabled parent の child、task-104 W1-8)	false にすると SessionStart の未処理 next-actions surface のみ止まる (discharge-guard 側は parent toggle で継続)	next-actions浮上有効化
...
review_iteration_min	reviewer_control	iter cycle 最小反復回数 (default 3、reviewer-count-guard.sh が本値未達 slug を advisory warn、iter 後半 CRIT 検出担保、task-101)	増やすと min iter 未達で advisory warn が出やすくなり iter cycle 継続を強く促す。減らすと閾値が緩み iter 1-2 での早期 closure が warn 対象外になる	レビュー反復下限
