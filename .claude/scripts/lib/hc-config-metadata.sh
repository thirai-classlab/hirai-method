# .claude/scripts/lib/hc-config-metadata.sh — task-48 Step 2 (TDD GREEN 前半)
#
# 目的:
...
#     CSV here-doc (`KEY|category|description|effect`) + grep/case で引く。
...
# ============================================================
# metadata table (CSV: KEY|category|description|effect)
# ============================================================
# 区切りは `|`。description / effect 内に `|` は使わない。
...
# CSV の指定 field を key で引く内部 helper
...
_hc_metadata_field() (
  ...
  local field="$2"
  local line
  line=$(_hc_metadata_table | grep -E "^${key}\|" | head -n 1) || true
  if [ -z "$line" ]; then
    return 1
  fi
...
hc_metadata_keys_by_category() (
  set -uo pipefail
  local cat="$1"
  # field 2 (category) が完全一致する行の field 1 (key) を出力
