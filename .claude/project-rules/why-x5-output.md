# project 固有ルール: why-x5-output (CommonRules / harness rule への追補)

> 本 file は **project 所有** (`install.sh --update` で上書きされない、なければ作成のみ)。
> `.claude/rules/why-x5-output.md` (harness 所有・update 上書き) から `@import` され、harness rule の**後に**結合 load される。
