# project 固有ルール: self-improvement (CommonRules / harness rule への追補)

> 本 file は **project 所有** (`install.sh --update` で上書きされない、なければ作成のみ)。
> `.claude/rules/self-improvement.md` (harness 所有・update 上書き) から `@import` され、harness rule の**後に**結合 load される。
