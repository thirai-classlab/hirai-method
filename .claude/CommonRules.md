...
- **`.claude/rules/*.md` / `.claude/commands/*.md` / `.claude/templates/docs/**/*.md` を draft 経由なしで直接 Edit/Write 禁止** → `.claude/hooks/draft-flow-guard.sh` 拡張 (task-40 で実装、`.claude/rules/*.md` 等の新 path pattern を block 対象に追加、対応 `docs/draft/<slug>.md` が `approved_at` 非空 or `retroactive: true` で pass)。bypass: `ECC_RULE_CHANGE_GUARD_OFF=1` / `HC_RULE_CHANGE_GUARD_ENABLED=false`。起源: 2026-05-26 task-40、本 session で規範変更時の draft skip 違反が発生 → retroactive リカバリ後に hook 拡張で再発防止
