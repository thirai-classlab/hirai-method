...
> **旧 hook BLOCK 委譲教訓 / 事故事例 / 真因 / 規範化経緯の詳細** (main/stg* push 禁止 / `.claude/` 配下 code Edit/Write 禁止 / `docs/` 直下 draft 経由必須 / Loop モード確認質問禁止 等、hook で機械防止済) は `.claude/hooks/{autonomous-action-guard,delegation-guard,draft-flow-guard,loop-confirmation-detector}.sh` source + `.claude/rules/{development-process,workflow,modes}.md` + memory `feedback_*.md` + `git log` 参照。本 table は honor system 教訓 (hook で防げないもの) に絞る。task-51 Step 2 で旧 5 bullet を本 1 行に圧縮 (2026-05-28、削減 -1K tokens)。
