...
- **適切な粒度でコミット**（必須）: 論理単位（1 機能 / 1 修正 / 1 リファクタ）ごとに `git commit` を切り、各コミットは独立動作可能な状態を保つ。Conventional Commits 形式のメッセージで、問題時に `git revert` / `git reset --hard <sha>` で戻せるようにする。巨大コミット禁止。
