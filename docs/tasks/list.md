...
| 14 | 🔄 | byproduct-discharge | Custom-PM Case 5 follow-up: `/sc:` 残存解消 (2026-05-14 起案 + 承認、W0 進行中): task #7 W4 (`f063ff3` /sc:* → 自前 command 置換) の事後監視 smoke `custom-pm-commands-smoke.sh` Case 5 が 2026-05-13 pre-existing FAIL。真因 = 2 file (next-actions entry #9 + README.md) が allowed-files regex 外。案 C ハイブリッド (W1 allowlist 暫定拡張 → W2 entry #9 履歴化 + /sc: literal 除去 → W3 整理 + smoke 6/6 verify) を採用。subagent research confidence 0.85 (agentId `ac8dfc5e6bfd4d22e`) | #7, #10 | [task-14-custom-pm-case-5-followup.md](task-14-custom-pm-case-5-followup.md) |
