...
| 21 | 📝 | system-reminder-attention | System-reminder 過剰注入 attention dilution + Loop モードと draft フロー相反 修正 (Wave 0 hook タイミング根本再配置 + Wave 1 頻度間引き + Wave 2 modes.md 例外条項 + draft-flow-guard.sh 既配備 `6ed9337` + Wave 3 eval 検証)。draft-flow-guard.sh は本 session 完遂、残り 6.2 session 想定。設計起源: 2026-05-23 user 観察 + skill 採用 (agent-harness-construction / eval-harness) | — | [task-21-system-reminder-attention-fix.md](task-21-system-reminder-attention-fix.md) ← draft: [`docs/draft/system-reminder-attention-fix.md`](../draft/system-reminder-attention-fix.md) |
| 22 | 📝 | hook-reliability-uplift | 26 hooks の fail policy 統一 (set -uo pipefail) + jq guard 追加 + 主要 guard smoke 拡充 + observe.jsonl rotation + improvement-proposal cache。CLAUDE.md Critical Lessons HIGH (set -e leak) と整合、coverage 50% → 75% 目標、3.5 session 想定 | — | [task-22-hook-reliability-uplift.md](task-22-hook-reliability-uplift.md) ← draft: [`docs/draft/hook-reliability-uplift.md`](../draft/hook-reliability-uplift.md) |
| 23 | 📝 | recall-poc-recovery | recall_poc 復旧 (git init で observe 混線解消 + CLAUDE.md template 改訂 + 遡及 draft frontmatter 整備 + list.md 初期化)。設計 01-03 が docs/ 直下直接 Write された事案の構造解消、1.2 session 想定 | #21 | [task-23-recall-poc-recovery.md](task-23-recall-poc-recovery.md) ← draft: [`docs/draft/recall-poc-recovery.md`](../draft/recall-poc-recovery.md) |
| 24 | 📝 | taskmanagesystem-recovery | taskManageSystem 復旧 (draft-flow-guard 同期 + HC_PROJECT_ROOT 環境変数固定 + docs_approved_dir config 化 + parent/child COEXISTENCE.md + tasks.md archive)。subdir 配置の特殊構造に対応、1.4 session 想定 | — | [task-24-taskmanagesystem-recovery.md](task-24-taskmanagesystem-recovery.md) ← draft: [`docs/draft/taskmanagesystem-recovery.md`](../draft/taskmanagesystem-recovery.md) |
| 25 | 📝 | harness-foundation | Performance (mermaid local install / SessionStart 並列化 / observe sampling) + Documentation (CLAUDE.md placeholder / README install.sh rewrite / harness-audit --compare) + Refactor & Drift (大型 hook 分割 / 未着手 draft 90 日超 list / settings.local drift check / project_id fallback 改善) の 3 サブ epic クラスタ。4.3 session 想定 | — | [task-25-harness-foundation-improvements.md](task-25-harness-foundation-improvements.md) ← draft: [`docs/draft/harness-foundation-improvements.md`](../draft/harness-foundation-improvements.md) |
| 26 | 📝 | delegation-code-enforcement | `.claude/hooks/` `.claude/skills/` `.claude/scripts/` 配下の code (.sh/.py/.mjs 等) へのメイン直接 Edit/Write を delegation-guard.sh で BLOCK 拡張 (config 化 `protected_paths_code` + `code_file_extensions`) + 規範 DRY 化 (development-process / task-management / workflow / modes の重複セクション統合)。本 session 中の規範違反 commit `6ed9337` `6561475` `17c493e` を起源として根本解決、context 複雑化も同時対処。2.4 session 想定 | — | [task-26-delegation-code-enforcement.md](task-26-delegation-code-enforcement.md) ← draft: [`docs/draft/delegation-code-enforcement.md`](../draft/delegation-code-enforcement.md) |
