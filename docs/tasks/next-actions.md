...
| 18 | 2026-05-23 | **observe.sh jq parse 失敗修復 (元前提 56% → 実測 0.04%)** — observations.jsonl の `--argjson raw` 経路で **jq parse 失敗が cascade** していた問題。root cause = literal control char (U+0000-U+001F) で `jq -c '.'` 自体が fail → `raw_safe` が error 文字列にすり替わり `--argjson` も連鎖失敗。**該当箇所**: `.claude/skills/continuous-learning-v2/hooks/observe.sh` の `raw_safe=$(... \| jq -c '.')` セクション | task-25 A3 subagent a85993694d32e78bf 計測中に発見 (2026-05-23、confidence 0.92)、A3 が報告した 56% は jq stream cascade fail を真の invalid 数と誤認した推定値、W2 subagent ae89946fa1ed81309 が Python json.JSONDecoder.raw_decode loop で実測したところ 11/28583 = 0.04% (drop 5600x) | 🔴→🟢 | task-27 として 化済 (`docs/tasks/task-27-...`)、W1 + W2 完遂 (commit `c25f3ee` + `fd5f6e5`)。draft 内 §1 / §4 の 56% 前提と DoD「jq-valid 率 44% → 95%+」は誤前提に基づく数値、実測値 0.04% を base に再評価必要。本 entry は **draft 修正 + harness-audit の jq-valid 率指標自体の見直し** を未処理項目として継続 | 🔄 → task-27 W1+W2 完遂 (`c25f3ee` + `fd5f6e5`) + 🚨 実測値で前提 5600x 誤認発見、draft / harness-audit 指標見直しは残作業 |
