...
| 18 | 2026-05-23 | **observe.sh jq parse 失敗 56% 修復** — observations.jsonl の `--argjson raw` 経路で **3916/7048 records (56%)** が jq-invalid として記録、L4 continuous-learning の学習データの半分が破損 / 解析不能。具体仮説: hook が渡す `raw` JSON 内の nested string escape (改行 / quote / unicode) が `--argjson` で再 escape されず JSON syntax error 化、fallback `{}` に潰れている。**該当箇所**: `.claude/skills/continuous-learning-v2/hooks/observe.sh` の `raw_safe=$(... \| jq -c '.')` セクション + 周辺の jq pipeline。L4 学習能力に直接影響、放置で confidence accumulation が半減し続ける | task-25 A3 subagent a85993694d32e78bf 計測中に発見 (2026-05-23、confidence 0.92) | 🔴 | (a) draft 起こし `observe-jq-parse-fix` 推奨 — schema 健全性確保 (raw JSON の安全な passthrough + jq invalid 行の origin / payload size / event type 統計取得 + 修復) + smoke 拡充 (3 種類の nested escape edge case + jq-valid 率 95%+ 目標) | 🔄 → [`docs/draft/observe-jq-parse-fix.md`](../draft/observe-jq-parse-fix.md) (2026-05-23、3 Wave 0.8 工数、案 C ハイブリッド = rawfile 経路 + 既存 invalid 行修復 script + L4 学習側 fromjson 適用)、user 承認待ち |
